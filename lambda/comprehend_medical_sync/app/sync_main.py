...
def lambda_handler(event, _):
    ...
    try:
        ...
        for entity in response['Entities']:
            if entity['Score'] > 0.50:
                classification_result = "{}-{}-{}.".format(entity['Category'], entity['Type'], entity['Id'])
                break
    ...
    except comprehend_medical.exceptions.TooManyRequestsException:
    ...
    except comprehend_medical.exceptions.TextSizeLimitExceededException as e:
        logger.error(e, exc_info=True)
        send_failure_to_step_function(e, 'TextSizeLimitExceededException',
                                      str(e), token, event)
    except comprehend_medical.exceptions.InvalidRequestException as e:
        logger.error(e, exc_info=True)
